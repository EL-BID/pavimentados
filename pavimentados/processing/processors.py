...
def draw_outputs(img, outputs, classes_labels, final_classes=None):
    ...
    for i in range(len(boxes)):
        ...
        cv2.rectangle(
            img, (label_x, label_y - label_height), (label_x + label_width, label_y + 10), color, cv2.FILLED
        )
...
class Image_Processor(Config_Basic):
    ...
    def load_models(self):
        ...
        self.yolov8_paviment_model = YoloV8Model(device=self.yolo_device, artifacts_path=self.artifacts_path,
    ...
    #     thresholds = [
    #         np.array([self.config["thressholds"][selection_type].get(elem, 0.2) for elem in classes[j]]) for j in
    #         range(len(classes))
    #     ]
    ...
    def crop_img(self, box, img):
        ...
        try:
            img_crop = (
                    cv2.resize(img_crop, tuple(self.siamese_model.config["SIAMESE_IMAGE_SIZE"])[:2],
...
class MultiImage_Processor(Group_Processor):
    ...
    def _process_batch(self, img_batch, video_output=None, image_folder_output=None):
        ...
        boxes_pav, scores_pav, classes_pav = self.processor.yolov8_paviment_model.predict(img_batch, conf=0.1, iou=0.2)

        # boxes_signal, scores_signal, classes_signal = self.processor.select_detections(prediction[1], "signals")
        boxes_signal, scores_signal, classes_signal = self.processor.yolov8_signal_model.predict(img_batch, conf=0.1, iou=0.2)
        ...
        if (video_output is not None) or (image_folder_output is not None):
            ...
            for img in img_batch:
                ...
                #                    self.processor.yolo_model.classes_paviment)
                img = draw_outputs(img, ([boxes_pav[j]], [scores_pav[j]], [classes_pav[j]]),
                                   self.processor.yolov8_paviment_model.classes_names)

                # img = draw_outputs(img, ([boxes_signal[j]], [scores_signal[j]], [classes_signal[j]]),
                #                    self.processor.yolo_model.classes_signal, final_signal_classes[j])
                img = draw_outputs(img, ([boxes_signal[j]], [scores_signal[j]], [classes_signal[j]]),
                                   self.processor.yolov8_signal_model.classes_names, final_signal_classes[j])
    ...
    def process_images_group(
        ...
        return {
            ...
            "final_pav_clases": [
                [self.processor.yolov8_paviment_model.classes_idx_names.get(elem, "<UNK>") for elem in item]
